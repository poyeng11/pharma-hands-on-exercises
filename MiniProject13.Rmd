...
mutate(perc = round((n/N)*100, digits=1), perc_char = format(perc, nsmall=1), npct = paste(n, 
 ...
 ungroup() %>% select(TRT01A, RACE, npct) %>% pivot_wider(names_from = TRT01A, values_from = npct)
...
adsl_mrg_cnt <- small_n_cnt %>%
  ...
  ungroup() %>% select(TRT01A, RACE, npct) %>%
  pivot_wider(names_from = TRT01A, values_from = npct)
...
derive_cnt  <- function(data,category,...) {
...
## Calculating count for each category 
  ...
  adsl_mrg_cnt<-small_n_cnt %>%
    ...
    mutate(perc = round((n/N)*100, digits=1), 
           ...
           npct = paste(n, paste0( "(", perc_char, ")" ))) %>%
    mutate(...) %>% 
    ungroup() %>% select(TRT01A, {{category}}, npct) %>% 
    pivot_wider(names_from=TRT01A,values_from=npct)
  
  adsl_mrg_cnt
}

derive_cnt(adsl_saf,category=SEX ,SEX = recode(SEX,
                                              "M" = "Male",
                                              "F" = "Female"))

derive_cnt(adsl_saf,category=AGEGR1)
derive_cnt(adsl_saf,category=RACE)
