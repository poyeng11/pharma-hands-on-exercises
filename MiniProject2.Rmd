...
adsl_eff <-   adsl %>%
 # filter(EFFFL == "Y" )
  filter(casefold(EFFFL, upper=TRUE) == "Y" )
...
#big N count with AGEGRP
N_CNT <- adsl_eff %>%
  group_by(TRT01AN, TRT01A,SEX) %>%
  count(name = "N")
N_CNT

#small n count with AGEGRP
n_CNT <- adsl_eff %>%
...
adsl_mrg_cnt2 <- n_CNT  %>%
  ...
  mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female")) %>%
  ungroup() %>%
  select(TRT01A,SEX, AGEGR1, npct) %>% pivot_wider(names_from = TRT01A, values_from = npct)
...
#combine
adae_mrg_cnt <- small_n  %>%
  left_join(Big_N) %>%
  mutate(perc = round(n/N*100, digits=1)) %>%
  mutate(perc_char = format(perc, nsmall=1)) %>%
  mutate(npct = paste(n, "(", perc_char, ")" ) ) %>%
  ungroup() %>%
  select(TRTA,AEBODSYS, npct) %>% pivot_wider(names_from = TRTA, values_from = npct)
