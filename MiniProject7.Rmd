...
ADLB_Plot <- function(mydata,x_label,y_label) {
  
 ALT2 <- read_xpt({mydata}) %>%
  filter(PARAMCD == "ALT")%>%
  filter(VISITNUM > 3) %>%
  mutate(WEEK = floor(ADY/7)) %>%
  filter(WEEK %in% c(0, 1, 2, 4, 8, 16)) %>%
  mutate(OUTRANGT = factor(LBNRIND, 
...
ADLB_Plot(inFile,"Alanine Aminotransferase U/L","Week")

ADLB_Plot(inFile,"Alanine Aminotransferase (U/L)","Study Week")
```
